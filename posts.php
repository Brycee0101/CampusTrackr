...
<?php
  ...
  $sessionName = $_SESSION['fname'];
  $query = "SELECT * FROM track_found WHERE found_email LIKE '%$sessionEmail%' AND found_stat = 'Reported'";
...
?>
    <section class="section-4">
        <div class="heading text-center mb-4 pt-5">
          <h2 class="heading pt-5">
            Posts from <?php echo $sessionName; ?>
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <form action="posts.php" method="post">
            ...
            <input type="submit" name="submit" value="Returned ✔" class="btn learn-more">
        </form>
        </div>
...
<?php
    if (isset($_POST['submit'])) {
        ...
        $query = "UPDATE track_found SET found_stat = 'Returned' WHERE found_id = $itemId";
        $result = mysqli_query($db, $query);
        if ($result) {
            ?><script>
                echo <script>alert('Message Sent!');
                window.location="posts.php";</script>
            </script><?php
        } else {
